...
<script>
  ...
  import {delArticleAPI, getDraftArticleListAPI, updateCategoryByIdAPI} from '@/api/article'
  export default {
    data () {
      return {
        emptyText: '暂无数据',
    ...
    methods: {
      handleEdit (index, row) {
        this.$confirm('要编辑这篇文章?', '提示', {
          ...
          .then(() => {
            this.$router.push({name: 'edit-article', query: {id: row._id, draft: 'yes'}})
          })
          .catch(() => {
            this.$message({
              type: 'success',
      ...
      updateArticleList () {
        ...
        getDraftArticleListAPI(reqParams)
          .then((res) => {
